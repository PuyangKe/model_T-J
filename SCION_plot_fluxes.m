...
%%%% T-J_data
plot(T_J_data.age, T_J_data.d13C,'.','color',[0.6510, 0.7255, 0.4235])
%%%% plot data comparison
...
%%%% plot data comparison
%%%% T-J_data
plot(T_J_data.age, T_J_data.d15N,'.','color',[0.6510, 0.7255, 0.4235])
%%%% plot N isotope proxy
%%%% plot this model
plot(state.time_myr,state.d15N_ocean,'k') ;
% plot(state.time_myr,state.d15N_ocean_1,'k') ;
% plot(state.time_myr,state.d15N_ocean_2,'r');
colormap(gca,'gray')
...
ylabel('Q_{norm}')
yyaxis right
plot(state.time_myr, state.fraction_water_column, 'b')
ylabel('f_{wc}')
title('Runoff (Q_{norm}) and water-column denitrification fraction')
...
plot(state.time_myr, state.N_volc, 'm')
legend({'Riverine N','Volcanic N'}, 'Location','best')
title('External N inputs')
