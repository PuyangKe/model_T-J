...
forcings.volcanics = [
   ...
   -218.0   0                -5 ;
   -214.0   1.45e10          -5 ;   % Angayucham 7 Myr，~ 1.45e10
   -207.0   0                -5 ;
...
C_volc        = interp1qr( forcings.volcanics(:,1), forcings.volcanics(:,2), t_geol );
delta_C_volc  = interp1qr( forcings.volcanics(:,1), forcings.volcanics(:,3), t_geol );
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% === Global land-area-weighted runoff and normalization ===
Qland_past    = RUNOFF_past  .* land_past  .* GRID_AREA_km2;
Qland_future  = RUNOFF_future.* land_future.* GRID_AREA_km2;
landarea_past = land_past    .* GRID_AREA_km2;
landarea_future = land_future.* GRID_AREA_km2;

Qmean_past    = sum(Qland_past(:))   / sum(landarea_past(:));
Qmean_future  = sum(Qland_future(:)) / sum(landarea_future(:));
Qmean         = contribution_past*Qmean_past + contribution_future*Qmean_future;

Qnorm = max(Qmean / pars.Qref_runoff, 0);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%% runoff in mm/yr
% QGRID_AREA_km2 = INTERPSTACK.gridarea_past = RUNOFF_past ;
% Q_past(Q_past<0) = 0 ;
...
% volcanic N
N_volc = pars.k_volcN * DEGASS;

% river N：basic * runoff * plant
riverine_input = pars.k_riverine * (1 + pars.riv_runoff_scale*(Qnorm - 1)) * (0.5 + 0.5*min(VEG,2));
riverine_input = max(riverine_input, 0);
...
%%%% Nitrate
dy(11) = nfix - denit_sediment - denit_water - monb + riverine_input + N_volc;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
eps_wc  = pars.eps_denit_wc;
eps_sed = pars.eps_denit_sed;
eps_ass = pars.eps_assim;

% low NO3-, inhibited Assimilation fractionation 
eps_ass_eff = eps_ass * ( Nconc / (Nconc + pars.K_NO3) );

d15N_denit_wc  = d15N_ocean - eps_wc;     % water column denitrification
d15N_denit_sed = d15N_ocean - eps_sed;    % Sedimentary denitrification
d15N_biomass   = d15N_ocean - eps_ass_eff;% biotic assimilation

d15N_atm = 0; % atmospheric N2 ~0‰
% Riverine δ15N responds to normalized runoff Qnorm
d15N_RIV = pars.d15N_riv_base + pars.alpha_riv_d15N*(Qnorm - 1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%% delta_A * A
dy(16) = -locb*(  delta_locb ) -mocb*( delta_mocb ) + oxidw*delta_G + ocdeg*delta_G + ccdeg*delta_C + carbw*delta_C - mccb*delta_mccb - sfw*delta_mccb + reductant_input*-5  + C_volc * delta_C_volc;
...
dy(22) = nfix*d15N_atm - denit_water*d15N_denit_wc - denit_sediment*d15N_denit_sed - monb*d15N_biomass + riverine_input*d15N_RIV + N_volc*pars.d15N_volcN ;
...
%%%%%%%%%% New  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    workingstate.Qnorm(stepnumber,1)              = Qnorm;
    workingstate.riverine_input(stepnumber,1)     = riverine_input;
    workingstate.N_volc(stepnumber,1)             = N_volc;
    workingstate.d15N_RIV(stepnumber,1)           = d15N_RIV;
    workingstate.d15N_biomass(stepnumber,1)       = d15N_biomass;
    workingstate.d15N_denit_wc(stepnumber,1)      = d15N_denit_wc;
    workingstate.d15N_denit_sed(stepnumber,1)     = d15N_denit_sed;
